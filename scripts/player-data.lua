...
---@field trip_view {key: string, index: integer, object_id: uint64|nil, on_start: boolean|nil, network_id: uint|nil}|nil -- Which of an
...
function player_data.update_settings(player, player_table)
  if  player and player.valid and player_table then
    ...
    local settings = {
      ...
      show_trip_estimates = mod_settings["li-show-trip-estimates"].value ~= false,
    }
...
--- Forget everything the window remembers about the network the player just left. Call this
--- whenever the player's network changes.
---
--- Rows skip an update when the data's generation counter has not moved, but those counters are
--- per network: two networks can be on the same count, and a row would then keep showing the one
--- the player just left. The trip on the map belongs to the old network the same way, and its
--- drawing outlives the move
---@param player_table PlayerData
---@return nil
function player_data.invalidate_ui_state(player_table)
